import React from 'react';
import './HouseMap.scss';
import LazyMap from '../../../../components/LazyMap/LazyMap';

const HouseMap: React.FC = () => {
  return (
    <div className="map-section">
      <h3>Наше расположение</h3>
      <div className="map-placeholder">
        <a href="https://yandex.ru/maps/org/7_kontinent/201915250712/?utm_medium=mapframe&utm_source=maps" className="map-placeholder__link map-placeholder__link--first">7 Континент</a>
        <a href="https://yandex.ru/maps/11010/republic-of-dagestan/category/resort/184106400/?utm_medium=mapframe&utm_source=maps" className="map-placeholder__link map-placeholder__link--second">База, дом отдыха в Республике Дагестан</a>
        <LazyMap 
          src="https://yandex.ru/map-widget/v1/?ll=47.935889%2C42.488504&mode=search&oid=201915250712&ol=biz&sctx=ZAAAAAgBEAAaKAoSCUZ4exAC3ENAEdqSVRFunEdAEhIJDJQUWABT1j8ResTouYWuwj8iBgABAgMEBSgKOABA%%2B0L3RgtC40L3QtdC90YKKAgCSAgCaAgxkZXNrdG9wLW1hcHM%3D&sll=47.935889%2C42.488504&sspn=0.042705%2C0.019416&text=7%20%D0%9A%D0%BE%D0%BD%D1%82%D0%B8%D0%BD%D0%B5%D0%BD%D1%82&z=15.03" 
        />
      </div>
    </div>
  );
};

export default HouseMap;
